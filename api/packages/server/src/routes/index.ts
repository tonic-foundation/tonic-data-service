export { default as monitoringRoutes } from './monitoring';

export { default as apiV1Routes } from './api-v1';
export { default as coinMarketCapV1Routes } from './external/coinmarketcap-v1';
export { makeTvRouter } from './tv';

export { default as leaderboardRoutes } from './leaderboard';
export { rewardsV1Routes, rewardsV2Routes, rewardsV3Routes, rewardsV4Routes } from './rewards';
export { rebatesV1Routes } from './rebates';
